...
public class UserDetailsActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.d(LoginActivity.TAG, "On create in user details");
    ...
    public void onResume() {
        ...
        Log.d(LoginActivity.TAG, "On Resume in UserDetails");
        ParseUser currentUser = ParseUser.getCurrentUser();
        if (currentUser != null) {
            // Check if the user is currently logged
            // and show any cached content
            Log.d(LoginActivity.TAG, "Updating views");
            updateViewsWithProfileInfo();
        } else {
            // If the user is not logged in, go to the
            // activity showing the login view.
            Log.d(LoginActivity.TAG, "Starting login activity");
            startLoginActivity();
        }
    }
                    ...
                    public void onCompleted(GraphUser user, Response response) {
                        if (user != null) {
                            ...
                            if ((response.getError().getCategory() == FacebookRequestError.Category.AUTHENTICATION_RETRY)
                                ...
                                Log.d(LoginActivity.TAG, "The facebook session was invalidated.");
                                onLogoutButtonClicked();
                            } else {
                                Log.d(LoginActivity.TAG, "Some other error: " + response.getError().getErrorMessage());
                            }
                        }
                    }
                });
    ...
    private void updateViewsWithProfileInfo() {
        Log.d(LoginActivity.TAG, "In Update Views");
        ParseUser currentUser = ParseUser.getCurrentUser();
        if (currentUser.get("profile") != null) {
            ...
            try {
                ...
                Log.d(LoginActivity.TAG, "Error parsing saved user data.");
            }
